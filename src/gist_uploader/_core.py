...
def upload_to_gist(
    ...
    *,
    ...
    token : str, optional
        GitHub personal access token; falls back to ``GIST_TOKEN`` environment variable.
    ...
    token = token or os.getenv("GIST_TOKEN")
    if not token:
        raise RuntimeError("GitHub token not provided. Set GIST_TOKEN environment variable.")
